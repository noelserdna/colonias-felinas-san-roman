...
`demo-personas`, `webpush`, `push-endpoint`, `questions-io`, `anexos-pdf`. When adding logic, put the decision part
...
**Configuration lives in the `settings` table** (key → JSON): course parameters (`settings.ts`, zod with defaults),
`branding` (municipio, provincia, contact, `credito_formativo`), `page:mi-colonia` (guide markdown), `mail_from`, and
...
**Ordinance-specific pieces** (San Román's ordinance, BOP Toledo n.º 122/2024): default colony guide
(`colonia.ts`), `seed/documents.json`, Anexo PDFs (`anexos-pdf.ts`; regenerate `public/docs/*.pdf` with
